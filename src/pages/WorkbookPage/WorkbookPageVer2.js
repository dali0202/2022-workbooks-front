...
function WorkbookPage() {
  ...
  const [lastWorkbookId, setLastWorkbookId] = useState(2000);
  const [workbookList, setWorkbookList] = useState([]);
  const obsRef = useRef(null);
  const cursor = useRef(2000);
  ...
  const getWorkbookList = async () => {
    const response = await requestGetWorkbookList2({ lastWorkbookId, keyword });
    cursor.current = response.data[response.data.length - 1].id;
    console.log(`api완료 cursur세팅${cursor.current}`);
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (target.isIntersecting) {
          console.log(`커서감지 현cursor:${cursor.current}`);
